...
func (s *Storage) GetAuthConfig() AuthConfig {
	enabled, _ := s.GetSettingsValue(authEnabledKey).(bool)
	username := strings.TrimSpace(s.GetSettingsValueString(authUsernameKey))
	password := s.GetSettingsValueString(authPasswordKey)
